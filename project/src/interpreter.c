...
int
interpreter (char *command_args[], int args_size)
{
  int i;
...
int
set (char *var, char *value[], int value_size)
{
...
int
ls_filter (const struct dirent *d)
{
  if (d->d_name[0] == '.')
    return 0;
  return 1;
...
int
ls_compare_str (const char *a, const char *b)
{
...
int
ls_compare (const struct dirent **a, const struct dirent **b)
{
...
void
runSchedule (struct queue *q, const struct schedule_policy *policy)
{
...
struct PCB *
run_pcb_to_completion (struct PCB *pcb)
{
  while (pcb_has_next_instruction (pcb))
    {
      ...
      if (instr == (size_t) -1)
	{
	  // Page fault occurred, process needs to be rescheduled
	  return pcb;
	}
...
struct PCB *
run_pcb_for_n_steps (struct PCB *pcb, size_t n)
{
...
int
my_exec (char *args[], int args_size)
{
  ...
  for (int n = 0; n < args_size; ++n)
	...
	{
	  struct PCB *pcb = pass_clone (q, args[n]);
